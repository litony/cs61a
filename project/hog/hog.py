...
def piggy_points(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    squareScore = score * score
    last, first = squareScore % 10, squareScore // 10
    while first > 0:
        if first % 10 < last:
            last = first % 10
        first //= 10
...
def more_boar(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    while player_score >= 100 or opponent_score >= 100:
        if player_score >= 100:
            player_score //= 10
        if opponent_score >= 100:
            opponent_score //= 10
    first1, last1 = player_score // 10, player_score % 10
    first2, last2 = opponent_score // 10, opponent_score % 10
    if first1 < first2:
        if last1 < last2:
            return True
        else:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if not who:
            score = take_turn(strategy0(score0, score1), score1, dice, goal)
            score0 += score
            if not more_boar(score0, score1):
                who = next_player(who)
        else:
            score = take_turn(strategy1(score1, score0), score0, dice, goal)
            score1 += score
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def announce_highest_helper(score0, score1):
        if not who:
            current_score = score0
        else:
            current_score = score1
        before_score, max_running = last_score, running_high
        sub_score = current_score - before_score
        if sub_score > max_running:
            print("Player {0} has reached a new maximum point gain. {1} point(s)!".format(who, sub_score))
            max_running = sub_score
        return announce_highest(who, current_score, max_running)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    min_num_rolls, num = 1, 1
    max_score = 0
    while num <= 10:
        total = make_averaged(roll_dice, trials_count)(num, dice)
        if  total > max_score:
            max_score = total
            min_num_rolls = num
        num += 1
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    havemore = more_boar(score, opponent_score)
    #print(havemore)
    if havemore:
        return 0
    elif piggy_points(opponent_score) >= cutoff and not havemore:
        return 0
    else:
        return num_rolls
